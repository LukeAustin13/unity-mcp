---
name: sw-rules-guard
description: Pre-flight and post-edit compliance check for States At War — enforces GameManager authority, turn-flow gates, combat trigger model, FactionId rules, input system constraints, prohibited changes, project scope, and assembly/namespace hygiene. Run this skill before any substantive edit to a core system, and again before marking a task complete. Its checks are READ-class, so it runs in any safety mode (read_only included); applying a fix it flags is a separate write. PROJECT-SPECIFIC EXAMPLE — before using in another project, replace the gates with your own project's non-negotiable rules; keep the pre-flight/post-edit gate structure and the block-and-explain contract.
license: MIT
---

Apply the project's non-negotiable rules from `CLAUDE.md` before edits happen (to catch prohibited directions early) and after edits are drafted (to catch accidental rule breaks). Each gate below is a hard check — not a suggestion. If a gate fails, block the change and explain exactly which rule was violated and what the compliant alternative is.

---

## Safety posture and how to run the checks

Every gate here is a **READ-class** check: it inspects source and reports Pass/Warn/Block. It runs in **any** safety mode, `read_only` included — the checks never mutate the project (`find_in_file` and `manage_script` reads are READ-class; the editor resources are reads). This skill decides *whether an edit is allowed and reports the verdict*; it does not perform the fix. Applying a compliant alternative it flags is a separate **write**, done by the editing agent (see `unity-mcp-operator` for the operate loop and `unity-edit-verify` for the edit-then-verify loop) and only in `write` mode. Check posture with `safety_status()` once if you are unsure; you never need `write` mode to run this gate.

**Running a detection pattern.** Each gate below gives a regex. `find_in_file` searches **one file** over MCP:

```python
find_in_file(uri="Assets/Scripts/AI/AIController.cs",
             pattern="Faction\\.(Player|Neutral|Enemy|Computer|None)",
             ignore_case=False,   # C# identifiers are case-sensitive; avoid false positives
             max_results=200)
```

There is no multi-file glob — no `files=` and no `limit=` parameter. Pass a single script's Assets-relative path (or a `mcpforunity://path/Assets/...` / `file://...` uri), and cap with `max_results`. For a **repo-wide sweep** across every `*.cs`, use your editor/repo code-search (ripgrep/grep) with the same regex, then use `find_in_file` on the specific script you are about to edit to get the exact line numbers. Post-edit, re-run each applicable gate's regex against the file(s) you actually changed.

**When a Block claims "won't compile"** (e.g. Gate 4): the C# edit tools already trigger import + compilation, so don't call `refresh_unity` to confirm — poll the `editor_state` resource's `compilation.is_compiling` until false, then `read_console(types=["error"])`. That verification is `unity-edit-verify`'s job; this gate only flags the regex-level regression.
...
Detection regex (repo-wide via code-search, or per-file via `find_in_file` with `ignore_case=False`):
```
Faction\.(Player|Neutral|Enemy|Computer|None)
```
...
Detection regex (repo-wide via code-search, or per-file via `find_in_file` with `ignore_case=False`):
```
UnityEngine\.Input\.|Input\.GetMouse|Input\.GetKey|Input\.mousePosition|StandaloneInputModule
```
...
Detection regexes (repo-wide via code-search, or per-file via `find_in_file` with `ignore_case=False` and `max_results` set to what you need):
```
FindObjectOfType|FindFirstObjectByType|FindAnyObjectByType|GameObject\.Find
\.material\s*=|\.sharedMaterial\s*=
\.Where\(|\.Select\(|\.ToList\(|\.OrderBy\(
GetComponent\s*<
```

For each hit, verify the enclosing method. A hit inside `Awake`, `Start`, `OnEnable`, or an event handler is not a violation. A hit inside `Update`, `FixedUpdate`, `LateUpdate`, or a method demonstrably called from one of those (trace the call chain by searching for the containing method name — `find_in_file` on the file that defines it) is a Block. These are heuristics: report a hot-path hit as advisory ("worth confirming the call chain") until you have traced it to a per-frame path.
...
Never add `Co-Authored-By: Claude` or any variant of Claude attribution to commit messages, branch names, or PR descriptions. This is a hard user preference (see project memory `feedback_no_coauthor`).
...
Apply `CLAUDE.md` "Smallest viable change." If you notice adjacent improvements while working on the requested task, list them as separate follow-up items. Do not silently include them in the current change unless they are required for correctness of the requested feature.
...
Detection workflow:
1. Before creating a new file, confirm the existing convention: search `^namespace\s+` across the target folder's `*.cs` with your code-search, or run `find_in_file(uri="Assets/Scripts/Combat/<AnExistingScript>.cs", pattern="^namespace\\s+")` on a neighbouring file in that folder.
