...
<ui:UXML xmlns:ui="UnityEngine.UIElements" xmlns:uie="UnityEditor.UIElements"
    ...
    <ui:Style src="project://database/Assets/UI/Styles/tokens.uss" />
    <ui:Style src="project://database/Assets/UI/Styles/components.uss" />
...
- Use `<ui:Style ...>` (with the `ui:` namespace prefix) to reference a stylesheet, **not** bare `<Style>`. UI Builder fails to open files that use `<Style>` without the prefix. Author these files with `manage_ui(action="create", path="...uxml", ...)` and link stylesheets with `manage_ui(action="link_stylesheet", ...)` — see `mcp-cookbook.md` Pattern 9. `.uxml`/`.uss` are never created with `create_script` (that tool is `.cs`-only).
