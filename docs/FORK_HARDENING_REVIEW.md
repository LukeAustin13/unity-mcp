...
**Wave 2 — flagship project intelligence (`manage_project`, READ, C#-backed) — landed (Python verified; C# pending your Unity run):** a new READ-classified `manage_project` tool with four actions: `validate_assets` (missing scripts, broken prefab instances, dangling serialized references across prefabs + ScriptableObjects on disk), `validate_materials` (missing / error / unsupported shaders — the "pink material" case), `asset_inventory` (counts by type, optional GUIDs), and **`project_health`** (aggregate asset+material report with per-category `healthy` flags). Scans are synchronous, paged (`page_size` + `cursor` → `next_cursor`), and capped (`max_issues`). A key design finding gates the shape: **the safety layer gates tools, not resources**, so these are tools classified READ (a resource would bypass the gate and be invisible to the safety table). Today only the *active scene's root objects* are checked — nothing else scans assets on disk. Python plumbing (`services/tools/manage_project.py`, CLI `unity-mcp project …`) is unit-tested; the C# scanner (`MCPForUnity/Editor/Tools/ManageProject.cs`) ships with EditMode tests under `TestProjects/UnityMCPTests` and is marked **needs Unity verification** — the Python side proves the wiring, the asset-walk itself must be confirmed in the Editor.

**Wave 3 — efficiency (landed; Python verified, C# review-clean pending your Unity run):**
- **`get_editor_context`** resource (`mcpforunity://editor/context`) — one read unions editor state + selection + prefab-stage + console error/warning counts, replacing ~4 task-start reads. C# `EditorContext.cs`, Python `services/resources/editor_context.py`, EditMode test. READ-classified.
- **`validate_build`** — READ-only build pre-flight (target resolves + build support installed, build scenes exist/enabled, no compile errors, PlayerSettings sane) so an agent can check "would this build succeed?" in `review_only` before a DESTRUCTIVE `manage_build`. C# `ValidateBuild.cs`, Python `services/tools/validate_build.py`, CLI `unity-mcp build validate`, Python + EditMode tests.
- **Additive `properties` bags** on `manage_gameobject`/`manage_ui` — an optional dict that carries the rarely-used long-tail params (via a `merge_properties` helper in `utils.py`), explicit params winning. **Purely additive** — existing calls forward byte-identical params (confirmed by review + tests).

**Wave 4 — orchestration skills (landed):** `unity-health-check` (drives `safety_status` + `editor/context` + `manage_project(project_health)` + `read_console` + `run_tests_and_summarize`), a generic safety-aware `unity-scene-setup`, and a Safety Mode Awareness section added to the shipped `unity-mcp-skill`. A stale duplicate of the operator guide at repo-root `unity-mcp-skill/` was identified for removal (left for maintainer confirmation — it is git-tracked).

**Wave 5 — the agentic loop (landed; Python verified, C# review-clean pending your Unity run):** the three primitives that turn the bridge from a puppet interface into a self-verifying development loop:

- **`manage_checkpoint`** — named scene-state snapshots (`create`/`list`/`restore`/`delete`) under `Library/McpCheckpoints/`. Create captures the *in-memory* state of every open saved scene (`SaveScene(..., saveAsCopy: true)`); restore copies the snapshots back over the original scene files and reloads — explicitly classified DESTRUCTIVE (payload-aware classifier: list→READ, create→VALIDATE, delete→WRITE, restore→DESTRUCTIVE, unknown→DESTRUCTIVE fail-closed). Ids are server-generated or sanitized to `^[A-Za-z0-9_-]{1,64}$` with a canonical-path prefix check — no user-supplied paths ever touch the filesystem. CLI: `unity-mcp checkpoint …`. Cap 20 checkpoints; untitled scenes reported as skipped.
- **`play_smoke_test`** (VALIDATE) — one call: enter play mode, run N seconds (default 15, cap 120), collect errors/exceptions, exit, return a pass/fail verdict. Because play entry triggers a domain reload that destroys in-memory state, this is a SessionState-persisted job with an `[InitializeOnLoad]` handler that re-arms after every reload, re-baselines console counts after `EnteredPlayMode` (handles Clear-on-Play), and finalizes on `EnteredEditMode`; a 30s grace timeout fails the job if play never enters (compile error). Python composes start→poll with backoff that tolerates the reload gap. CLI: `unity-mcp playtest run`.
- **Dependency X-ray on `manage_project`** (READ) — `get_dependencies` (forward, path or GUID, optional recursive), `find_references` (reverse: which assets depend on the target, scan-capped with `scanned_count`/`truncated`), and advisory `unused_assets` (roots = enabled build scenes + `Resources/` + `StreamingAssets`; excludes scripts/folders/`Editor/`; ships a `caveats` list — Addressables/bundles/reflection are invisible to static analysis; never deletes). CLI: `unity-mcp project deps/refs/unused`.

**Wave 6 — perception & audit (landed; Python verified, C# review-clean pending your Unity run):** built from the principle *ask the engine before you look at pixels* — Claude's vision downscales images to ~1.15 MP, so high-res inline captures are pure payload waste:

- **Tiered visual audit on `manage_camera`** — Tier 1 `visibility_report` (READ): frustum test + screen-space rect % + coverage + optional single-ray occlusion for every renderer, zero pixels shipped. Tier 2 `screenshot_compare` (VALIDATE): capture vs baseline file → `changed_pixel_pct`, diff bounding box, mean delta — numbers, not images (baseline path validated against the screenshot/Assets sandbox). Tier 3 right-sized capture: inline base64 now defaults to `max_width=1280` (the full-resolution file on disk is unchanged), plus JPEG option and `crop_target` object-focused crops. A payload-aware `_classify_manage_camera` makes capture/report actions usable outside write mode (fail-closed WRITE for unknown actions).
- **`query_scene`** (READ) — "SQL for the scene": filter (name/tag/layer/component/root-path/scene) + projection (`transform`, `world_bounds` i.e. shape dimensions with Renderer→Collider→RectTransform fallback, `components`, `materials`, `mesh_stats`) over all loaded scenes in one paged, deterministic call. Replaces the find→per-object-inspect round-trip pattern. CLI: `unity-mcp query scene`.
- **`audit_mobile` on `manage_project`** (READ) — 18 data-driven mobile-performance rules: texture/audio/model import settings, IL2CPP + ARM64 + graphics APIs, quality settings (MSAA/shadows/pixel lights), active-scene red flags. Each finding carries `rule_id`/`category`/`severity`/`fix`; each settings probe is individually try/caught into `rule_errors` so one API mismatch can't kill the audit. CLI: `unity-mcp project audit-mobile`.
- **`read_console` `format="summary"`** — Python-side dedup/grouping of console output (signature-normalized, counts + sample + file:line), capped at 25 groups; existing formats untouched.
- **Test triage on `run_tests_and_summarize`** — optional `include_source_context` (±5 source lines around each failing line, parsed from stack traces, ≤10 failures, silently skipped when files aren't locally readable) and `retry_failed` (≤3 re-runs of only the failed tests via the `testNames` filter, classifying `flaky` vs `deterministic_failures`). Defaults are byte-identical to prior behavior.

**Wave 7 — content contracts & UI intelligence (landed; Python verified, C# review-clean pending your Unity run):** tests check code; these check *content* — the thing Unity projects actually rot through. All three are READ-classified, work in `read_only` mode, never mutate scenes/assets/settings, never enter play mode, and fail closed on unknown actions:

- **`audit_ui_layout`** (READ) — uGUI layout audit across a configurable resolution matrix (default 720p/1080p/1440p/ultrawide, strict `WxH` parse, ≤8 entries). Deliberately **analytic**: CanvasScaler scale factors are computed with Unity's exact per-mode formulas and RectTransform screen rects with pure anchor/pivot/offset math — no GameView resolution switching, no `ForceUpdateCanvases`, so nothing is ever dirtied. 11 checks: `off_screen`, `overlapping_interactive` (pair-capped), `tiny_target`, `clipped_text`, `invisible_interactable`, `blocked_interactable`, `suspicious_anchors`, `canvas_scaler`, `missing_listeners`, `missing_event_system`, `missing_graphic_raycaster`. Findings carry severity/scene/object path/screen rect/resolution/reason/suggested fix; anything layout-group-driven or heuristic is `advisory: true`. Facts only — no screenshots (use `manage_camera` `crop_target` for pixels on demand). CLI: `unity-mcp ui audit-layout`.
- **`validate_scene_contracts`** (READ) — scene rules as code: a JSON contract (inline or a `.json` under the project root — canonical-prefix-checked, 256 KB cap) declares `required_objects`, `forbidden_objects`, `required_components`, `max_cameras`/`max_lights`, `forbid_missing_references`, `required_tags`/`required_layers`, `require_in_build_settings`, and `ui` requirements. Unknown top-level keys are refused outright — a typo'd rule can never silently pass. Validates **loaded** scenes only; a named-but-unloaded scene is a structured error, never an implicit scene load. Returns a full pass/fail checklist with `passed` = zero failures. CLI: `unity-mcp scene validate-contracts`.
- **`prefab_health` on `manage_project`** (READ) — deep prefab validation on disk via `AssetDatabase.LoadAssetAtPath` traversal only (no `LoadPrefabContents`, no instantiation): missing scripts, dangling serialized refs, broken variants, conservative duplicate-component detection, undefined tags/unnamed layers, null material slots/error shaders, disabled colliders/renderers (advisory), nested-prefab depth, oversized renderer bounds, plus first-level `dependencies` in single-prefab mode. GUID-cursor paged over a folder; per-check try/catch into `rule_errors` (audit_mobile resilience pattern). CLI: `unity-mcp project prefab-health`.

Residual risks (Wave 7): the analytic UI math intentionally diverges from live layout wherever LayoutGroup/ContentSizeFitter drive sizes — those findings are advisory by construction, not silently wrong; uGUI/TMP types are resolved by reflection (neither the editor asmdef nor the EditMode test asmdef references `UnityEngine.UI`), so a missing UI package degrades to an explicit "check skipped" finding rather than a compile break; `blocked_interactable` draw-order is an approximation (sort order + paint index) and only ever advisory; `contract_path` gives the C# side a sandboxed file-read primitive — bounded by the project-root canonical-prefix check, `.json` suffix, and size cap.

Waves 2–3 and 5–7 include C# that needs a live Unity Editor to verify end-to-end (Python tests cover plumbing/paging/classification; the new handlers reuse only API patterns present in existing compiling files); they ship with EditMode tests under `TestProjects/UnityMCPTests`. Suite after Wave 7: **1798 passed, 3 skipped**.
